    ...
    if action == "extend":
        ...
        with _TASK_LOCK:
            ...
            if task:
                ...
                task["state_history"].append({"state": "failed", "at": _utc_now_iso(), "by": "user", "error": "cancelled by Susi"})
                _persist_tasks()
        ...
        _ws_broadcast("task_failed", {"task_id": task_id, "agent_id": esc.get("agent_id", ""), "error": "cancelled by Susi"})  # type: ignore[misc]
