...
| `bridge_escalation_resolve` | `bridge_escalation_resolve(task_id: str, action: str, reassign_to: str = '', extend_minutes: int = 30)` | Resolve a Stage 3 escalation (Susi-Entscheidung). Actions: extend (give more time), reassign (assign to another agent), cancel (abort tas... | `bridge_mcp.py:2340` |
...
| `bridge_email_send` | `bridge_email_send(to: str, subject: str, body: str)` | Send an email through the Bridge email system. Creates an approval request — the email is only sent after Leo approves. Returns immediate... | `bridge_mcp.py:2814` |
...
| `bridge_slack_send` | `bridge_slack_send(channel: str, message: str)` | Send a message to a Slack channel through the Bridge. Creates an approval request — Leo must approve before the message is sent. After ap... | `bridge_mcp.py:3058` |
...
| `bridge_stealth_start` | `bridge_stealth_start(proxy: str = '', user_agent: str = '', headless: bool = True, profile: str = '')` | Start a stealth browser session using Playwright (bot-detection bypass). Returns session_id for subsequent calls. Max 3 concurrent sessio... | `bridge_mcp.py:4498` |
...
| `bridge_cdp_tabs` | `bridge_cdp_tabs()` | List all open tabs in Leo's browser with URLs and titles. | `bridge_mcp.py:5221` |
| `bridge_cdp_navigate` | `bridge_cdp_navigate(url: str, tab_index: str = '0:0')` | Navigate a tab in Leo's browser to a URL. Specify tab index (from bridge_cdp_tabs) or uses active tab. | `bridge_mcp.py:5252` |
| `bridge_cdp_screenshot` | `bridge_cdp_screenshot(tab_index: str = '0:0', full_page: bool = False)` | Take a screenshot of a tab in Leo's browser. Returns file path to saved screenshot. | `bridge_mcp.py:5273` |
| `bridge_cdp_click` | `bridge_cdp_click(selector: str, tab_index: str = '0:0')` | Click an element by CSS selector in Leo's browser. | `bridge_mcp.py:5291` |
| `bridge_cdp_fill` | `bridge_cdp_fill(selector: str, value: str, tab_index: str = '0:0')` | Fill an input field by CSS selector in Leo's browser. | `bridge_mcp.py:5308` |
| `bridge_cdp_evaluate` | `bridge_cdp_evaluate(expression: str, tab_index: str = '0:0')` | Execute JavaScript on a page in Leo's browser. Returns result. | `bridge_mcp.py:5325` |
| `bridge_cdp_content` | `bridge_cdp_content(tab_index: str = '0:0')` | Get the HTML content of a page in Leo's browser. | `bridge_mcp.py:5342` |
| `bridge_cdp_disconnect` | `bridge_cdp_disconnect()` | Disconnect from Leo's browser. Does NOT close the browser. | `bridge_mcp.py:5368` |
| `bridge_cdp_new_tab` | `bridge_cdp_new_tab(url: str = 'about:blank')` | Open a new tab in Leo's browser and navigate to a URL. Returns the new tab index. | `bridge_mcp.py:5392` |
| `bridge_cdp_close_tab` | `bridge_cdp_close_tab(tab_index: str)` | Close a specific tab in Leo's browser by index. Use bridge_cdp_tabs to find indices. | `bridge_mcp.py:5417` |
| `bridge_cdp_file_upload` | `bridge_cdp_file_upload(selector: str, file_path: str, tab_index: str = '0:0')` | Upload a file via a file input element in Leo's browser (CDP). Selector should target an <input type='file'> element. | `bridge_mcp.py:5438` |
| `bridge_browser_open` | `bridge_browser_open(url: str = 'about:blank', engine: str = 'auto', headless: bool = True, proxy: str = '', user_agent: str = '', profile: str = '')` | Open a unified browser session. Choose engine: 'stealth' (anti-detection Playwright), 'cdp' (Chrome DevTools, Leo's browser), or 'auto' (... | `bridge_mcp.py:5645` |
...
| `bridge_browser_fingerprint_snapshot` | `bridge_browser_fingerprint_snapshot(session_id: str)` | Capture a browser-level fingerprint snapshot from a unified browser session. Useful for anti-detection lab validation and regression trac... | `bridge_mcp.py:6912` |
...
| `bridge_phone_call` | `bridge_phone_call(number: str)` | Start an outgoing phone call via the Voice Gateway. Requires Leo's approval before the call is placed. Returns the approval request_id — ... | `bridge_mcp.py:7777` |
...
| `bridge_vision_act` | `bridge_vision_act(session_id: str, goal: str, max_steps: int = 10)` | Autonomous vision-action loop: takes a goal and executes browser actions to achieve it. Requires an active stealth browser session. Loop:... | `bridge_mcp.py:8250` |
...
- Gefundene Markdown-Dateien im Release-Clone (`/home/leo/Desktop/bridge-ide-release/BRIDGE`): 1
  - `CLAUDE.md`
