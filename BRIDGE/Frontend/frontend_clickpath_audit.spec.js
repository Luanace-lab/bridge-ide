...
test.describe('frontend clickpath audit', () => {
  ...
  test('project config page executes safe clickpaths and exports JSON', async ({ page }) => {
    const tmpRoot = `/tmp/bridge_frontend_audit_${Date.now()}`;
    ...
    await page.locator('#projPath').fill('/tmp/bridge-test');
    ...
    await expect(page.getByText('Projekt-Erstellung ist nur innerhalb des Projektverzeichnisses erlaubt.')).toBeVisible();
