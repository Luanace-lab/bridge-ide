...
This note is the running, verified gap register for the working copy in `/home/user/bridge/BRIDGE`.
...
- `curl -fsS 'http://127.0.0.1:9111/agent/config?project_path=/home/user/bridge/BRIDGE&engine=claude'`
- `curl -fsS 'http://127.0.0.1:9111/agent/config?project_path=/home/user/bridge/BRIDGE&engine=codex'`
...
| high | BRIDGE | Hardcoded local machine paths across active team config | `Backend/team.json` binds projects and agents to `/home/user/bridge/...` and `/home/user/.claude-sub2` paths across many entries, including Buddy, Viktor, Nova, and project roots. | The working copy depends on one machine layout and one user profile, which weakens foreign-machine reproducibility. |
