...
| `project_config.html` | Scan existing project path | Works | Browser run on `/home/leo/Desktop/CC/BRIDGE` returned `Projekt erkannt. 23 von 42 Konfigurationen gefunden.` |
| `project_config.html` | Create project with `/tmp` as target path | Works with client-side guard | Browser run now shows `Projekt-Erstellung ist nur innerhalb von /home/leo/Desktop erlaubt.` before the POST path is used; the same audit run then created `/home/leo/Desktop/bridge_frontend_audit_<ts>/frontend-audit-<ts>` successfully |
